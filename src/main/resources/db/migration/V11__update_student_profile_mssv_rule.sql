...
ALTER TABLE student_profiles ADD COLUMN IF NOT EXISTS verified_student_code VARCHAR(30);

-- Add a partial unique index to ensure uniqueness only for verified codes
CREATE UNIQUE INDEX IF NOT EXISTS idx_student_profiles_verified_code ON student_profiles (verified_student_code) WHERE verified_student_code IS NOT NULL;
