...
public class BunnyWebhookEventsCleanupScheduler {
    ...
    public void executeCleanupJob() {
        ...
        if (retentionDays <= 0) {
            log.info("BunnyWebhookEvents cleanup is disabled because retentionDays <= 0");
            return;
        }

        log.info("Starting BunnyWebhookEvents cleanup job for records older than {} days", retentionDays);
        ...
        log.info("Finished BunnyWebhookEvents cleanup job. Deleted {} records in {} ms", totalDeleted, duration);
    }
}
