...
public class GoogleCalendarSyncService {
    ...
    private void handleCreate(GoogleCalendarSyncJob job, Booking booking, Session session) {
        ...
        boolean isManualMeeting = (session != null && !session.isGoogleCalendarManaged())
                || (session != null && session.getMeetingPlatform() != null && session.getMeetingPlatform() != MeetingPlatform.GOOGLE_MEET);
        String finalMeetUrl = isManualMeeting ? (session != null ? session.getMeetingLink() : null) : response.googleMeetUrl();
        eventLinkRepository.save(GoogleCalendarEventLink.builder()
                ...
                .build());
        markSynced(session, connection, finalMeetUrl);
    }
    ...
    private void handleUpdate(GoogleCalendarSyncJob job, Booking booking, Session session) {
        ...
        if (!isManualMeeting && response.googleMeetUrl() != null) {
    ...
    private void markSynced(Session session, GoogleCalendarConnection connection, String meetUrl) {
        Instant nowUtc = timeProvider.instant();
        if (session != null) {
            boolean isManualMeeting = !session.isGoogleCalendarManaged()
                    || (session.getMeetingPlatform() != null && session.getMeetingPlatform() != MeetingPlatform.GOOGLE_MEET);
            if (!isManualMeeting && meetUrl != null) {
