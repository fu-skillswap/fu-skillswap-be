...
public class PaymentOrder {
    ...
    private void syncShadowFields() {
        if (expiresAtUtc != null) {
            expiresAt = com.fptu.exe.skillswap.shared.time.BookingTime.fromInstant(expiresAtUtc);
        } else if (expiresAt != null) {
            expiresAtUtc = com.fptu.exe.skillswap.shared.time.BookingTime.toInstant(expiresAt);
        }

        if (paidAtUtc != null) {
            paidAt = com.fptu.exe.skillswap.shared.time.BookingTime.fromInstant(paidAtUtc);
        } else if (paidAt != null) {
            paidAtUtc = com.fptu.exe.skillswap.shared.time.BookingTime.toInstant(paidAt);
        }

        if (cancelledAtUtc != null) {
            cancelledAt = com.fptu.exe.skillswap.shared.time.BookingTime.fromInstant(cancelledAtUtc);
        } else if (cancelledAt != null) {
            cancelledAtUtc = com.fptu.exe.skillswap.shared.time.BookingTime.toInstant(cancelledAt);
        }

        if (failedAtUtc != null) {
            failedAt = com.fptu.exe.skillswap.shared.time.BookingTime.fromInstant(failedAtUtc);
        } else if (failedAt != null) {
            failedAtUtc = com.fptu.exe.skillswap.shared.time.BookingTime.toInstant(failedAt);
        }

        if (creditFinalizedAtUtc != null) {
            creditFinalizedAt = com.fptu.exe.skillswap.shared.time.BookingTime.fromInstant(creditFinalizedAtUtc);
        } else if (creditFinalizedAt != null) {
            creditFinalizedAtUtc = com.fptu.exe.skillswap.shared.time.BookingTime.toInstant(creditFinalizedAt);
        }

        if (releasedAtUtc != null) {
            releasedAt = com.fptu.exe.skillswap.shared.time.BookingTime.fromInstant(releasedAtUtc);
        } else if (releasedAt != null) {
            releasedAtUtc = com.fptu.exe.skillswap.shared.time.BookingTime.toInstant(releasedAt);
        }

        if (refundedAtUtc != null) {
            refundedAt = com.fptu.exe.skillswap.shared.time.BookingTime.fromInstant(refundedAtUtc);
        } else if (refundedAt != null) {
            refundedAtUtc = com.fptu.exe.skillswap.shared.time.BookingTime.toInstant(refundedAt);
        }
    }
}
