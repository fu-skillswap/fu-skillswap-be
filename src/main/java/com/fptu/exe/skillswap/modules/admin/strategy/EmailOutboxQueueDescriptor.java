...
@Component
public class EmailOutboxQueueDescriptor implements AdminQueueDescriptor {

    @Override
    public boolean supports(AdminQueueKey queueKey) {
        return queueKey == AdminQueueKey.EMAIL_OUTBOX_FAILED;
    }

    @Override
    public AdminCaseType resolveCaseType(AdminQueueKey queueKey) {
        return AdminCaseType.EMAIL_OUTBOX;
    }
    ...
    public String buildDetailPath(AdminQueueKey queueKey, String detailRefId) {
        return "/api/admin/email-outbox/" + detailRefId;
    }

    @Override
    public List<String> getAvailableActions(AdminQueueKey queueKey) {
        return List.of("VIEW_DETAIL", "ASSIGN_TO_ME", "RETRY_EMAIL");
    }
}
