...
public interface ConversationRepository extends JpaRepository<Conversation, UUID>, ConversationRepositoryCustom {
    ...
    @Query("SELECT c FROM Conversation c JOIN ConversationParticipant cp ON c.id = cp.conversation.id WHERE cp.user.id = :userId and cp.accessState <> com.fptu.exe.skillswap.modules.chat.domain.ConversationParticipantAccess.REVOKED ORDER BY c.lastMessageAt DESC NULLS LAST, c.createdAt DESC")
