...
class BookingNotificationIntegrationTest {
    ...
    void acceptBooking_shouldNotifyAcceptedMentee_andAutoRejectedSiblingMentees() {
        ...
        awaitNotificationCount(mentee3.getId(), 1);
    ...
    private void awaitNotificationCount(UUID userId, int expectedCount) {
        long deadline = System.currentTimeMillis() + 3000;
        while (System.currentTimeMillis() < deadline) {
            if (notificationService.getMyNotifications(userId, true, PageRequest.of(0, 10)).getTotalElements() >= expectedCount) {
                return;
            }
            try {
                Thread.sleep(50);
